...
public class HuanzheJiuzhiInfoAdapter extends BaseAdapter {
	private final static int MAX_SHOW_CHARAC = 20;
	Context context;
	List<HuanzheJiuzhiInfo> list;
	Handler handler;

	public HuanzheJiuzhiInfoAdapter(Context context, List<HuanzheJiuzhiInfo> list, Handler handler) {
	...
	private void setItemData(ViewHolder holder, int position) {
		...
		holder.first.setText(list.get(position).getItemJiuzhiShijian());
		holder.second.setText(list.get(position).getItemJiuzhiJianyaoJilu());
		
		String tmp = list.get(position).getItemJiuzhiCaozuoren();
